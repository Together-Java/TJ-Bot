...
public final class FreeCommand extends SlashCommandAdapter implements EventReceiver {
     ...
     */
    @Override
    public void onSlashCommand(@NotNull final SlashCommandInteraction event) {
     ...
     */
    private boolean handleShouldBeProcessed(@NotNull final SlashCommandInteraction event) {
        if (!isReady) {
            logger.debug(
    ...
    private @NotNull Guild requiresGuild(SlashCommandInteraction event) {
