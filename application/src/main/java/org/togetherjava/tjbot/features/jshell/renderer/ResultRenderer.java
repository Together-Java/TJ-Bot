...
public class ResultRenderer {
    ...
    private final ResultMinimalEmbedRenderer minimalEmbedRenderer = new ResultMinimalEmbedRenderer();

    /**
     * Renders a JShell result.
     *
     * @param gistApiToken the token to use to send the result to gist, in case it is too big for an embed
     ...
     * @return the result
     */
    public MessageEmbed render(String gistApiToken, @Nullable Member originator, boolean showCode, JShellResult result) {
    ...
    private MessageEmbed renderFailure(JShellResult result) {
        ...
        return new EmbedBuilder().setTitle("Couldn't render the result, please contact a moderator.").setColor(Color.RED).build();
    }
