...
/**
 * Error envelope attached to a {@link ToolResult}. Serialized as JSON and returned to the model so
 * it can recover (retry, try a different tool, or answer without the missing data) rather than the
 ...
 *        {@code null}
 */
public record ToolError(boolean isError, @Nullable String errorMessage) {
     ...
     */
    public static ToolError none() {
        return new ToolError(false, null);
    }
    ...
    public static ToolError of(String message) {
        return new ToolError(true, message);
    }
}
